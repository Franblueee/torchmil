...
@patch("os.path.isdir")
@patch("torchmil.datasets.wsi_dataset.WSIDataset.__init__", return_value=None)
def test_load_labels_directory_mode(
    mock_super_init, mock_isdir, mock_super_load, base_kwargs
):
    """Test directory mode logic. Manually set labels_path since super().__init__ is mocked."""
    mock_isdir.return_value = True

    ds = TridentWSIDataset(**base_kwargs)
    ds.labels_path = base_kwargs[
        "labels_path"
    ]  # Manually set attribute usually set by super()
...
@patch("os.path.isdir")
@patch("torchmil.datasets.wsi_dataset.WSIDataset.__init__", return_value=None)
def test_load_labels_csv_mode_success(
    mock_super_init, mock_isdir, mock_read_csv, base_kwargs
):
    """Test CSV mode logic. Manually set labels_path."""
    mock_isdir.return_value = False

    df = pd.DataFrame({"filename": ["slide_1", "slide_2"], "grade": [0, 1]})
...
@patch("os.path.isdir")
@patch("torchmil.datasets.wsi_dataset.WSIDataset.__init__", return_value=None)
def test_load_labels_csv_missing_kwargs(
    mock_super_init, mock_isdir, mock_read_csv, base_kwargs
):
    mock_isdir.return_value = False
    ...
    with pytest.raises(
        ValueError, match="must provide 'wsi_name_col' and 'wsi_label_col'"
    ):
...
@patch("os.path.isdir")
@patch("torchmil.datasets.wsi_dataset.WSIDataset.__init__", return_value=None)
def test_load_labels_csv_not_found(
    mock_super_init, mock_isdir, mock_read_csv, base_kwargs
):
    mock_isdir.return_value = False
...
def test_load_coords_calculation(mock_super_init, mock_h5, base_kwargs):
    ...
    raw_coords = np.array([[1024, 2048], [1536, 2560]])
